...
int make_bytecode(){
	...
	while(j<prog_len){
		...
		if (strchr(&prog[j][0], ':')){ //check if the TEXT_POS(pc) contains a label
			if (sscanf(&prog[j][0],"%" XSTR(MAX_ARG_LEN) "[^:]: %" XSTR(MAX_ARG_LEN) "s %" XSTR(MAX_ARG_LEN) "s %" XSTR(MAX_ARG_LEN)
				"s %" XSTR(MAX_ARG_LEN) "s", label, opcode, arg1, arg2, arg3) < 2){ //parse the TEXT_POS(pc) with label
					printf("ERROR: parsing TEXT_POS(pc) %d\n", j);
					return(-1);
			}
		}
		else {
			if (sscanf(&prog[j][0],"%" XSTR(MAX_ARG_LEN) "s %" XSTR(MAX_ARG_LEN) "s %" XSTR(MAX_ARG_LEN)
				"s %" XSTR(MAX_ARG_LEN) "s", opcode, arg1, arg2, arg3) < 1){ //parse the TEXT_POS(pc) without label
					printf("ERROR: parsing TEXT_POS(pc) %d\n", j);
					return(-1);
			}
		}
		...
		for (i=0; i<MAX_OPCODE; i++){
					...
					printf("ERROR: TEXT_POS(pc) %d opcode error (assembly: %s %s %s %s)\n", j, opcode, arg1, arg2, arg3);
					return(-1);
				}
				else {
					printf("0x%08x 0x%08x\n",ADDR_TEXT + 4*j, bytecode);
					text[j] = bytecode; //bytecodes saved in there.
                               		break;
				}
                	}
			if(i==(MAX_OPCODE-1)) {
				printf("ERROR: TEXT_POS(pc) %d unknown opcode\n", j);
				return(-1);
			}
        	}
		j++;
...
int exec_bytecode(){
	...
	//while loop will stop when element is text[] array has no bytecode.
	...
	//add
	if ((text[TEXT_POS(pc)] & 0xFC0007FF) == 0x20){

	  registers[(text[TEXT_POS(pc)] & 0x0000F800) >> 11] =
		registers[(text[TEXT_POS(pc)] & 0x001F0000) >> 16] +
		registers[(text[TEXT_POS(pc)] & 0x03E00000) >> 21];
	...
	//addi
	else if ((text[TEXT_POS(pc)] & 0xFC000000) == 0x20000000){

	  registers[(text[TEXT_POS(pc)] & 0x001F0000) >> 16] =
		registers[(text[TEXT_POS(pc)] & 0x03E00000) >> 21] +
		(text[TEXT_POS(pc)] & 0xFFFF);
  ...
  //andi
	...
	//srl
  else if ((text[TEXT_POS(pc)] & 0xFC00003F) == 0x2){

	registers[(text[TEXT_POS(pc)] & 0x0000F800) >> 11] =
	registers[(text[TEXT_POS(pc)] & 0x001F0000) >> 16] >>
	((text[TEXT_POS(pc)] & 0x7C0) >> 6 );
	...
	//sll
	else if ((text[TEXT_POS(pc)] & 0xFC00003F) == 0x0){
	...
	//beq
	else if ((text[TEXT_POS(pc)] & 0xFC000000) == 0x10000000){

		if  (registers[(text[TEXT_POS(pc)] & 0x001F0000) >> 16] ==
		     registers[(text[TEXT_POS(pc)] & 0x03E00000) >> 21]){

			short int num=(text[TEXT_POS(pc)] & 0xFFFF);

			pc= pc + (num*4) + 4;	   // newPC  =  oldPC + 4 + num*4
...
//bne
	...
	//NOP
	else if (text[TEXT_POS(pc)] == 0x00000000){
